...
tf.app.flags.DEFINE_integer('how_many_training_steps', 500,
...
tf.app.flags.DEFINE_integer('train_batch_size', 1,
...
def add_evaluation_step(result_tensor, ground_truth_tensor):
  ...
  with tf.name_scope('accuracy'):
    with tf.name_scope('correct_prediction'):
      ...
      predicted_tensor = tf.round(result_tensor)
      correct_prediction = tf.equal(tf.round(predicted_tensor), ground_truth_tensor)
      TP = tf.count_nonzero(predicted_tensor * ground_truth_tensor, 1)
      TN = tf.count_nonzero((predicted_tensor - 1) * (ground_truth_tensor - 1), 1)
      FP = tf.count_nonzero(predicted_tensor * (ground_truth_tensor - 1), 1)
      FN = tf.count_nonzero((predicted_tensor - 1) * ground_truth_tensor, 1)
      precision = tf.divide(TP, TP + FP)
      recall = tf.divide(TP, TP + FN) 
      '''
      F1_num = 2.0 * precision * recall
      F1_denom = precision + recall
      F1 = tf.divide(F1_num, F1_denom)
      '''
    with tf.name_scope('accuracy'):
      ...
      match_accuracy = tf.reduce_mean(tf.cast(correct_prediction, tf.float32))
      precision_rate = tf.reduce_mean(precision)
      recall_rate = tf.reduce_mean(recall)
    tf.summary.scalar('match_accuracy', match_accuracy)
    tf.summary.scalar('precision', precision_rate) 
    tf.summary.scalar('recall', recall_rate)
  return (ground_truth_tensor, predicted_tensor, match_accuracy, precision_rate, recall_rate)
...
def main(_):
  ...
  for i in range(FLAGS.how_many_training_steps):
    ...
    if (i % FLAGS.eval_step_interval) == 0 or is_last_step:
      (ground_tensor, predicted_tensor, match_accuracy, precision_rate, recall_rate), cross_entropy_value = sess.run(
          [evaluation_step, cross_entropy],
          feed_dict={bottleneck_input: train_bottlenecks,
                     ground_truth_input: train_ground_truth})
      
      print("ground_tensor: ", ground_tensor)
      print("pred_tensor: ", predicted_tensor)
      oneIndices = [1 for i in range(class_count)]
      condition_ground = tf.equal(ground_tensor, oneIndices)
      condition_pred = tf.equal(predicted_tensor, oneIndices)
      indices_ground = tf.where(condition_ground)
      ones_ground = sess.run([indices_ground])
      print("indices of one for ground_tensor: ", ones_ground)
      print("ones_ground size: ", len(ones_ground[0]))
      indices_match = tf.where(condition_pred)
      ones_pred = sess.run([indices_match])
      print("indices of one for predicted_tensor: ", ones_pred)
      print("ones_pred size: ", len(ones_pred[0]))
      print('%s: Step %d: Train accuracy = %.1f%%' % (datetime.now(), i,
                                                      match_accuracy * 100))
      print('%s: Step %d: Train precision = %.1f%%' % (datetime.now(), i,
                                                      precision_rate * 100))
      print('%s: Step %d: Train recall = %.1f%%' % (datetime.now(), i,
                                                      recall_rate * 100))
      ...
      # with the `merged` op.
      validation_summary, (ground_tensor, predicted_tensor, match_accuracy, precision_rate, recall_rate) = sess.run(
          [merged, evaluation_step],
          feed_dict={bottleneck_input: validation_bottlenecks,
                     ground_truth_input: validation_ground_truth})
      validation_writer.add_summary(validation_summary, i)

      print('%s: Step %d: Validation accuracy = %.1f%%' % (datetime.now(), i,
                                                      match_accuracy * 100))
      print('%s: Step %d: Validation precision = %.1f%%' % (datetime.now(), i,
                                                      precision_rate * 100))
      print('%s: Step %d: Validation recall = %.1f%%' % (datetime.now(), i,
                                                      recall_rate * 100))
